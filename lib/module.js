...
var generatorCellNumber = function generatorCellNumber(index, rowNumber) {
  return '' + generateColumnLetter(index) + rowNumber;
};

var generatorStringCell = function generatorStringCell(index, value, rowIndex) {
  return '<c r="' + generatorCellNumber(index, rowIndex) + '" t="inlineStr"><is><t>' + (0, _lodash2.default)(value) + '</t></is></c>';
};

var generatorNumberCell = function generatorNumberCell(index, value, rowIndex) {
  return '<c r="' + generatorCellNumber(index, rowIndex) + '"><v>' + value + '</v></c>';
};

var formatCell = function formatCell(cell, index, rowIndex) {
...
var generatorRows = function generatorRows(rows) {
...
var zipcelx = function zipcelx(config) {
  if (!validator(config)) {
